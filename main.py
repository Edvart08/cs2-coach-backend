...
"rating_breakdown":{{"aim":75,"game_sense":60,"consistency":55,"utility":40,"clutch":50}}
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.post("https://api.groq.com/openai/v1/chat/completions",
            ...
            json={"model":"llama-3.3-70b-versatile",
                "messages":[
                    {"role":"system","content":"""Ты CS2 тренер. Говоришь НАПРЯМУЮ с игроком на 'ты'.
ЗАПРЕЩЕНО: слова 'игрок', 'игроку', 'пользователь' — ТОЛЬКО 'ты', 'твой', 'тебе', 'у тебя'.
ОБЯЗАТЕЛЬНО: называй конкретные цифры из данных (K/D 0.85, WR 44%, карту de_dust2).
Если нет данных по картам — напиши 'нет данных по картам'.
Отвечай ТОЛЬКО валидным JSON без markdown. Русский язык."""},
                    {"role":"user","content":prompt}],
                "temperature":0.7,
                "max_tokens":1500,
                "response_format":{"type":"json_object"}})
